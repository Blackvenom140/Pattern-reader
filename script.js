...
async function getHistoricalResults(limit = 100) {
...
async function aiPredict(results) {
  // Convert to big/small sequence
  const bigSmallSeq = convertToBigSmall(results);

  // Get patterns and break points
  const patterns = identifyPatterns(bigSmallSeq);
  const breakPoints = detectBreakPoints(bigSmallSeq);
  ...
  // First generate base prediction
  let prediction = await predictNextOutcome(bigSmallSeq, patterns, breakPoints);

  // Apply breakpoint logic if lifecycle suggests it
  const patternStatus = patternTracker.isPatternStarting(patterns)
    ? "starting"
    : patternTracker.isPatternEndingOrBreaking(patterns, breakPoints)
    ? "ending"
    : "unknown";

  if (breakpointLogic.shouldUseBreakpoint(patternStatus) && breakPoints.length > 0) {
    const bp = breakPoints[0];
    if (bp.confidence > 0.7) {
      prediction = {
        value: bp.prediction,
        confidence: bp.confidence + 0.05,
        reasoning: "[Breakpoint Override] " + bp.reasoning
      };
    }
  }

  // Then apply LIVE AI override (if better)
  const liveTrend = analyzeLiveTrend(results);
  const liveAI = predictFromLiveAI(liveTrend);

  if (liveAI.confidence > prediction.confidence) {
    prediction = {
      value: liveAI.value,
      confidence: liveAI.confidence,
      reasoning: "[LIVE AI] " + liveAI.reasoning
    };
  }
  ...
  // If new pattern, save it to Firebase
  if (patterns.length > 0 && patterns[0].confidence > 0.7) {
    ...
    if (snapshot.exists()) {
      ...
      await savePatternToFirebase(patterns[0].type, bigSmallSeq, {
